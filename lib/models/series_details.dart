...
SeriesDetails seriesDetailsFromJson(String str) =>
    SeriesDetails.fromJson(json.decode(str));
...
class SeriesDetails {
  ...
  factory SeriesDetails.fromJson(Map<String, dynamic> json) => SeriesDetails(
    ...
    episodeRunTime: List<int>.from(
      (json["episode_run_time"] ?? []).map((x) => (x as num).toInt()),
    ),
...
class LastEpisodeToAir {
  ...
  factory LastEpisodeToAir.fromJson(Map<String, dynamic> json) =>
      LastEpisodeToAir(
        ...
        voteAverage: (json["vote_average"] as num?)?.toDouble() ?? 0.0,
        voteCount: (json["vote_count"] as num?)?.toInt() ?? 0,
        airDate: DateTime.parse(json["air_date"] ?? DateTime.now().toString()),
